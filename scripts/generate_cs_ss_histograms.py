    ...
    with open(fname, mode='r') as file:
        ...
        for lines in csvFile:
            if lines[1] in standard :
                if (lines[1],lines[2]) not in plot_data:
                    plot_data[(lines[1],lines[2])]=[[],[]]
                plot_data[(lines[1], lines[2])][0].append(float(lines[5]))
                plot_data[(lines[1], lines[2])][1].append(lines[6])
                if lines[2] == 'H':
                    plot_data[('x','H')][0].append(float(lines[5]))
                    plot_data[('x', 'H')][1].append(lines[6])
            try:
                atm[(lines[1],lines[2])][0].append(float(lines[5]))
                atm[(lines[1], lines[2])][1].append(lines[6])
            except KeyError:
                if lines[1] == "ALA" and lines[2] in ['HB1','HB2','HB3']:
                    atm[(lines[1], 'MB')][0].append(float(lines[5]))
                    atm[(lines[1], 'MB')][1].append(lines[6])
                if lines[1] == "ILE" and lines[2] in ['HD11','HD12','HD13']:
                    atm[(lines[1], 'MD')][0].append(float(lines[5]))
                    atm[(lines[1], 'MD')][1].append(lines[6])
                if lines[1] == "ILE" and lines[2] in ['HG21','HG22','HG23']:
                    atm[(lines[1], 'MG')][0].append(float(lines[5]))
                    atm[(lines[1], 'MG')][1].append(lines[6])
                if lines[1] == "LEU" and lines[2] in ['HD11','HD12','HD13']:
                    atm[(lines[1], 'MD1')][0].append(float(lines[5]))
                    atm[(lines[1], 'MD1')][1].append(lines[6])
                if lines[1] == "LEU" and lines[2] in ['HD21','HG22','HG23']:
                    atm[(lines[1], 'MD2')][0].append(float(lines[5]))
                    atm[(lines[1], 'MD2')][1].append(lines[6])
                if lines[1] == "MET" and lines[2] in ['HE1','HE2','HE3']:
                    atm[(lines[1], 'ME')][0].append(float(lines[5]))
                    atm[(lines[1], 'ME')][1].append(lines[6])
                if lines[1] == "THR" and lines[2] in ['HG21','HG22','HG23']:
                    atm[(lines[1], 'MG')][0].append(float(lines[5]))
                    atm[(lines[1], 'MG')][1].append(lines[6])
                if lines[1] == "VAL" and lines[2] in ['HG11','HG12','HG13']:
                    atm[(lines[1], 'MG1')][0].append(float(lines[5]))
                    atm[(lines[1], 'MG1')][1].append(lines[6])
                if lines[1] == "VAL" and lines[2] in ['HG21','HG22','HG23']:
                    atm[(lines[1], 'MG2')][0].append(float(lines[5]))
                    atm[(lines[1], 'MG2')][1].append(lines[6])
...
def read_csv3(fname):
    with open(fname, mode='r') as file:
        ...
        plot_data[('x','H')]= [[],[]]
        for line in csvFile:
            d = [line[0],line[1],line[2],line[3],line[4],line[6],line[7],line[8],line[9]]
            cs.append(",".join(d))
        fo = open('cs_filtered.csv','w')
        print (len(cs))
        cs2 = set(cs)
        print (len(cs2))
        for k in cs2:
